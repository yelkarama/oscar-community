...
public class OLISSearchAction extends DispatchAction {
	...
	public ActionForward loadResults(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
		...
		if (redo) {
			...
			Driver.submitOLISQuery(request, query, loggedInInfo.getLoggedInProvider());
		}
	...
	private Query createOlisQuery(String queryType, LoggedInInfo loggedInInfo, HttpServletRequest request) {
		...
		if (queryType.equalsIgnoreCase("Z01")) {
			...
			try {
				if (request.getParameterValues("requestingHic") != null && request.getParameterValues("requestingHic").length > 1) {
				...
				} else if (requestingHicProviderNo != null && requestingHicProviderNo.trim().length() > 0) {
					Provider provider = providerDao.getProvider(requestingHicProviderNo);
