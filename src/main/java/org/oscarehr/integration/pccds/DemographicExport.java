...
import org.oscarehr.common.dao.DiseaseRegistryPCDS_Dao;
...
//mport org.oscarehr.e2e.util.EverestUtils;
...
import oscar.oscarDemographic.pageUtil.DemographicExportForm;
//import oscar.oscarDemographic.pageUtil.PGPEncrypt;
import oscar.oscarDemographic.pageUtil.ReadHRMFile;
...
public class DemographicExport {
	public DemographicExport(Security SEC,Provider PROV){
		
		super();
		MiscUtils.getLogger().info("Creating Demographic Export 1");
		status = false;

		exPersonalHistory = false;     			// WebUtils.isChecked(request, "exPersonalHistory");
		exFamilyHistory = false;        			// WebUtils.isChecked(request, "exFamilyHistory");
		exPastHealth = false;           			// WebUtils.isChecked(request, "exPastHealth");  Changed to FALSE on 2017-08-10 per PM request
		exProblemList = false;					// WebUtils.isChecked(request, "exProblemList");   // changed to FALSE per request 2017-08-14 QA request
		exRiskFactors = false; 					// WebUtils.isChecked(request, "exRiskFactors");
		exAllergiesAndAdverseReactions = true; 	// WebUtils.isChecked(request, "exAllergiesAndAdverseReactions");
		exMedicationsAndTreatments = true; 		// WebUtils.isChecked(request, "exMedicationsAndTreatments");
		exImmunizations = true; 				// WebUtils.isChecked(request, "exImmunizations");
		exLaboratoryResults = false; 			// WebUtils.isChecked(request, "exLaboratoryResults");
		exAppointments = false;					// WebUtils.isChecked(request, "exAppointments");
		exClinicalNotes = false;				// WebUtils.isChecked(request, "exClinicalNotes");
		exReportsReceived = false;				// WebUtils.isChecked(request, "exReportsReceived");
		exAlertsAndSpecialNeeds = false; 		// WebUtils.isChecked(request, "exAlertsAndSpecialNeeds");
		exCareElements = true;					// WebUtils.isChecked(request, "exCareElements
		demographicManager = SpringUtils.getBean(DemographicManager.class);
		demographicDao = SpringUtils.getBean(DemographicDao.class);
		demoExpListDao = SpringUtils.getBean(DemographicToExportPCDS_Dao.class);
		demoDiseaseRegDao = SpringUtils.getBean(DiseaseRegistryPCDS_Dao.class);
		provider = PROV;
		security = SEC;
	...
	public DemographicExport(Security SEC,Provider PROV,boolean PersonalHistory, boolean FamilyHistory, boolean PastHealth, boolean ProblemList, boolean RiskFactors, boolean AllergiesAndAdverseReactions, boolean MedicationsAndTreatments, boolean Immunizations, boolean LaboratoryResults, boolean Appointments, boolean ClinicalNotes, boolean ReportsReceived, boolean AlertsAndSpecialNeeds, boolean CareElements){
		super();
		status = false;

		exPersonalHistory = PersonalHistory;     						// WebUtils.isChecked(request, "exPersonalHistory");
		exFamilyHistory = FamilyHistory;        						// WebUtils.isChecked(request, "exFamilyHistory");
		exPastHealth = PastHealth;           							// WebUtils.isChecked(request, "exPastHealth");
		exProblemList = ProblemList;									// WebUtils.isChecked(request, "exProblemList");
		exRiskFactors = RiskFactors; 									// WebUtils.isChecked(request, "exRiskFactors");
		exAllergiesAndAdverseReactions = AllergiesAndAdverseReactions; 	// WebUtils.isChecked(request, "exAllergiesAndAdverseReactions");
		exMedicationsAndTreatments = MedicationsAndTreatments;			// WebUtils.isChecked(request, "exMedicationsAndTreatments");
		exImmunizations = Immunizations; 								// WebUtils.isChecked(request, "exImmunizations");
		exLaboratoryResults = LaboratoryResults;						// WebUtils.isChecked(request, "exLaboratoryResults");
		exAppointments = Appointments;									// WebUtils.isChecked(request, "exAppointments");
		exClinicalNotes = ClinicalNotes;								// WebUtils.isChecked(request, "exClinicalNotes");
		exReportsReceived = ReportsReceived;							// WebUtils.isChecked(request, "exReportsReceived");
		exAlertsAndSpecialNeeds = AlertsAndSpecialNeeds; 				// WebUtils.isChecked(request, "exAlertsAndSpecialNeeds");
		exCareElements = CareElements;									// WebUtils.isChecked(request, "exCareElements	
		demographicManager = SpringUtils.getBean(DemographicManager.class);
		demographicDao = SpringUtils.getBean(DemographicDao.class);
		demoExpListDao = SpringUtils.getBean(DemographicToExportPCDS_Dao.class);
		demoDiseaseRegDao = SpringUtils.getBean(DiseaseRegistryPCDS_Dao.class);
	...
	/*
	 * The original way to call 
	 */
	public boolean exportData(String patientSet){
		DemographicExportForm defrm = new DemographicExportForm();
		defrm.setPatientSet(patientSet);
		return exportData(new DemographicSets().getDemographicSet(defrm.getPatientSet()));
	}
	
	/*
	 * The new way (generate list independent of the OSCAR patient set method)
	 */
	public boolean exportData(List<String> patientSet){
		/*
		 * Get instructions from Processor service (web service)
		 * - type of export
		 * - date range if required.
		 * 
		 * Allow comm module to control when the extract actually occurs (ie: when transmission is done)
		 * Auto optimize: just set this process to execute often and it will export when system can accept new export.
		 */
		boolean state = true;
		...
		if (DEBUG){
			logger.info("\tDEBUG : " + this.getClass().getName() + " : pcCDS Export job running as: " + loggedInInfo.getLoggedInProvider().getFormattedName());
			logger.info("\tDEBUG : exportData: MAX export (paging) : " + demoExpListDao.getPageValue());
			logger.info("\tDEBUG : exportData: TOTAL active patients : " + demoExpListDao.getTotalActive());
		...
		/* TODO
		 * Generate this list by another means (custom query)
		 * Identify patients from the log table (any interactions with a demographic number) 
		 * eg: select demographic_no from log where demographic_no IS NOT NULL AND dateTime BETWEEN '2016-05-01 00:00:00' AND '2016-06-01 00:00:00' GROUP BY demographic_no; 
		 */
		
		// replace this with a query that collects list of demographic numbers
		List<String> list = patientSet;
		...
		String ffwd = "fail";
		String tmpDir = oscarProperties.getProperty("PCDS_PROCESSING_DIR");
		
		// Sharing Center - holds the ID that will 'potentially' be exported.
		int documentExportId = 0;
		
		// only use template 0 (CMS4) removed switch from original code
		try {
			if (DEBUG){
				logger.info("\t\tDEBUG : " + this.getClass().getName() + " : Starting export to TMP_DIR=" + tmpDir);
			}
			DemographicData d = new DemographicData();
			Util.checkDir(tmpDir);
			...
			for (String demoNo : list) {
				if (StringUtils.empty(demoNo)) {
					...
					if (DEBUG){
						logger.info("\t\tDEBUG " + this.getClass().getName() + "Demographic value is  valid : "+demoNo+"  (pcCDS project)");
					}
					// DEMOGRAPHICS
					org.oscarehr.common.model.Demographic demographic = demographicManager.getDemographic(loggedInInfo, demoNo);
					
					if (DEBUG){
						if (demographic == null){
							logger.info("\tDEBUG " + this.getClass().getName() +" demographic is null (pcCDS project)");
						}else {
							logger.info("\tDEBUG " + this.getClass().getName() + "Accessed demographic info  via DAO :"+demographic.getFirstName()+ ", status="+ demographic.getPatientStatus()+"  (pcCDS project)");
						}
					}

					if (demographic.getPatientStatus()!=null) {   //  && demographic.getPatientStatus().equals("Contact-only")
						if (DEBUG){
							logger.info("\t\tDEBUG " + this.getClass().getName() + "Demographic has status");
						}
						...
						if (cdsDt.Gender.Enum.forString(sex)!=null) {
							...
							exportError.add("Error! No Gender for Patient or can't specify code. Adding as O to ensure element is introduced. "+demoNo);
							demo.setGender(cdsDt.Gender.Enum.forString("O"));
						...
						*/  // requeste removal by QA 2017-08-14
								...
								if (StringUtils.filled(concerns)) {
									...
									for (CaseManagementNoteExt cme : cmeList) {
										if (cme.getKeyVal().equals(CaseManagementNoteExt.PROBLEMDESC)) {
											...
											// PCDS: remove description
											
											if (StringUtils.filled(cme.getValue())) {
												//pList.setProblemDescription(cme.getValue());
												pList.setProblemDescription(RESTRICTED);
												//summary = Util.addSummary(summary, "Problem Description", cme.getValue()); //nullify 
												summary = Util.addSummary(summary, "Problem Description", RESTRICTED);
											}
											bPROBLEMDESC = true;
									...
									if (StringUtils.filled(annotation)) {
										...
										pList.setNotes(RESTRICTED);
										//summary = Util.addSummary(summary, "Notes", annotation);
										summary = Util.addSummary(summary, "Notes", RESTRICTED);
									}
									//pList.setCategorySummaryLine(summary);
									pList.setCategorySummaryLine("RESTRICTED BY CLINICAL DIRECTOR");
						...
						for (Map<String, Object> prevMap : prevList) {
							...
							if (Util_orig.isNonImmunizationPrevention(prevType)) {
								if (exPastHealth) {
									phSummary = null;
									PastHealth pHealth = patientRec.addNewPastHealth();
									...
									*/
									pHealth.setCategorySummaryLine("RESTRICTED BY CLINICAL DIRECTOR");
								}
								...
								prevType = Util_orig.getImmunizationType(prevType);
								...
								if (!arr[p].isArchived()){
									...
									if (StringUtils.filled(drugName)) {
										...
										if (StringUtils.filled(drugName)) {
											...
											medi.setDrugName(drugName); // request of PM: if no name, then set name as description (or custom name as it would seem).
											mSummary = Util.addSummary(mSummary, "Drug Description", drugName);
										} else {
											medi.setDrugDescription("No drug name or description on record");
											medi.setDrugName("No drug name or description on record");
						...
						// PCDS Add Disease registry items
						if (demoDiseaseRegDao.getDiseaseReg(demoNo, patientRec)){
							logger.info("Disease registry items successfully retrieved.");
						}else {
							logger.error("Disease registry items FAILED on export.");
						}
						...
						if (DEBUG){
							logger.info("\t\tDEBUG : " + this.getClass().getName() + " : pcCDS saving files to : " + tmpDir);
						}
						try{
							...
							File aFile = new File(directory, expFile+".xml");
						...
						try {
								...
								// the above permission setting seems to fail.  Using the sledgehammer to set permissions This file is temporary until transmission complete.
								// note: this is a linux command.  May have to add some logic to detect what OS is in future
								Runtime.getRuntime().exec("chmod 666 " + files.get(files.size()-1).getCanonicalPath());
					...
					if (DEBUG){
						logger.info("\t\tDEBUG : " + this.getClass().getName() + " : Made it to the end of the for loop!");
					}
				}
			
				String pcds_log_export = oscarProperties.getProperty("PCDS_LOG_EXPORT");

				if (pcds_log_export.length() > 0){
					logger.info("LOG EXPORT Return: " + sendHttpMsg(pcds_log_export + list.size()));
				}
				
				
				String pcds_transmit = oscarProperties.getProperty("PCDS_TRANSMIT");
				System.out.println("\t\tTransmission URL is: " + pcds_transmit);
				if (pcds_transmit != null && pcds_transmit.length() > 0){
					logger.info("TRANSMISSION STATUS : " + sendHttpMsg(pcds_transmit));
				}
			}catch(Exception e){
				e.printStackTrace();
			}
	...
	LoopContacts:
		for (int j=0; j<demographicRelationships.size(); j++) {
			if (StringUtils.filled(contactId[j])) {
				...
				//fillContactInfo(loggedInInfo, contact, contactId[j], demoNo, j);  disabled by request of QA
			}
		}
	}
...
//		  String info = labRoutingInfo.get("comment"); <--for whole report, may refer to >1 lab results
	...
	private void init(){
		try{
			MiscUtils.getLogger().info("Begin: Initilaizing demographic export class.");
			logger = MiscUtils.getLogger();
			demoArchiveDao = (DemographicArchiveDao) SpringUtils.getBean("demographicArchiveDao");
			contactDao = (DemographicContactDao) SpringUtils.getBean("demographicContactDao");
			partialDateDao = (PartialDateDao) SpringUtils.getBean("partialDateDao");
			hrmDocToDemographicDao = (HRMDocumentToDemographicDao) SpringUtils.getBean("HRMDocumentToDemographicDao");
			hrmDocDao = (HRMDocumentDao) SpringUtils.getBean("HRMDocumentDao");
			hrmDocCommentDao = (HRMDocumentCommentDao) SpringUtils.getBean("HRMDocumentCommentDao");
			cmm = (CaseManagementManager) SpringUtils.getBean("caseManagementManager");
			hl7TxtInfoDao = (Hl7TextInfoDao)SpringUtils.getBean("hl7TextInfoDao");
			hl7TxtMssgDao = (Hl7TextMessageDao)SpringUtils.getBean("hl7TextMessageDao");
			demographicExtDao = (DemographicExtDao) SpringUtils.getBean("demographicExtDao");
			...
			loggedInInfo.setLoggedInProvider(new Provider("1"));
			exportError = null;
			entries = new HashMap<String, Integer>();
			oscarProperties = OscarProperties.getInstance();
			
			logger.info("INIT " + this.getClass().getName() + " status: " + status + " (pcCDS project)");
		}catch (Exception e){
			e.printStackTrace();
		}
	...
	private boolean exPersonalHistory = true;     			// WebUtils.isChecked(request, "exPersonalHistory");
	private boolean exFamilyHistory = true;        			// WebUtils.isChecked(request, "exFamilyHistory");
	private boolean exPastHealth = true;           			// WebUtils.isChecked(request, "exPastHealth");
	private boolean exProblemList = true;					// WebUtils.isChecked(request, "exProblemList");
	private boolean exRiskFactors = true; 					// WebUtils.isChecked(request, "exRiskFactors");
	private boolean exAllergiesAndAdverseReactions = true; 	// WebUtils.isChecked(request, "exAllergiesAndAdverseReactions");
	private boolean exMedicationsAndTreatments = true; 		// WebUtils.isChecked(request, "exMedicationsAndTreatments");
	private boolean exImmunizations = true; 				// WebUtils.isChecked(request, "exImmunizations");
	private boolean exLaboratoryResults = true; 			// WebUtils.isChecked(request, "exLaboratoryResults");
	private boolean exAppointments = false;					// WebUtils.isChecked(request, "exAppointments");
	private boolean exClinicalNotes = false;				// WebUtils.isChecked(request, "exClinicalNotes");
	private boolean exReportsReceived = false;				// WebUtils.isChecked(request, "exReportsReceived");
	private boolean exAlertsAndSpecialNeeds = true; 		// WebUtils.isChecked(request, "exAlertsAndSpecialNeeds");
	private boolean exCareElements = true;					// WebUtils.isChecked(request, "exCareElements
	private Logger logger;
	private DemographicArchiveDao demoArchiveDao;
	private DemographicToExportPCDS_Dao demoExpListDao;
	private DemographicContactDao contactDao;
	private PartialDateDao partialDateDao = (PartialDateDao) SpringUtils.getBean("partialDateDao");
	private HRMDocumentToDemographicDao hrmDocToDemographicDao;
	private HRMDocumentDao hrmDocDao;
	private HRMDocumentCommentDao hrmDocCommentDao;
	private CaseManagementManager cmm;
	private Hl7TextInfoDao hl7TxtInfoDao;
	private Hl7TextMessageDao hl7TxtMssgDao;
	private DemographicExtDao demographicExtDao;
	private SecurityInfoManager securityInfoManager;
	private LoggedInInfo loggedInInfo;
	private DemographicManager demographicManager;
	private DemographicDao demographicDao;
	private DiseaseRegistryPCDS_Dao demoDiseaseRegDao;
	private Provider provider;
	private Security security;
	private Integer exportNo;
	private ArrayList<String> exportError;
	...
	public static final String RESTRICTED = "RESTRICTED BY CLINICAL DIRECTOR"
