        ...
        throws ServletException, IOException
    {
        ...
        boolean valid = true;
                ...
                for(int i=0; i<iType; i++){
                    ...
                    if(!ectValidation.isDate(dateObserved, "yyyy-MM-dd H:mm")&&inputValue.compareTo("")!=0){
                        errors.add(dateName,
                ...
                if(valid){
                    for(int i=0; i<iType; i++){
                        ...
                        if(!GenericValidator.isBlankOrNull(inputValue)){
                        	...
                        	sc.setDateObserved(ConversionUtils.fromDateString(dateObserved, "yyyy-MM-dd H:mm"));
                            ...
                            if(ms.isEmpty()){
                            	...
                            	m.setDateObserved(ConversionUtils.fromDateString(dateObserved, "yyyy-MM-dd H:mm"));
                            	m.setAppointmentNo(0);
                            	dao.persist(m);
                                
                                //prepare input values for writing to the encounter form
                                if (mFlowsheet == null){
                                    textOnEncounter =  textOnEncounter + inputType + ": " + inputValue + " " + mInstrc + (comments.isEmpty() ? " " : " - ") + comments + "\n";
                                }else{
                                    textOnEncounter += mFlowsheet.getFlowSheetItem(inputType).getDisplayName()+": "+inputValue + (comments.isEmpty() ? " " : " - ") +  comments + "\n";
                                }
                            }
                        }
