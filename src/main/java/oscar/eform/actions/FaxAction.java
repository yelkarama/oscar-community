...
public final class FaxAction {
	...
	public void faxForms(String[] numbers, String formId, String providerId) throws DocumentException {
		...
		try {
			...
			String existfilename = request.getParameter("existfilename");
			if(null != existfilename && existfilename.length() > 0){
				//File filename = File.createTempFile("EForm." + formId, ".pdf");
				String tmpPath = System.getProperty("java.io.tmpdir") + File.separator +"EForm." + formId + ".pdf";
				String filePath = tmpPath.substring(0, tmpPath.lastIndexOf(File.separator));
				File file = new File(filePath + File.separator + existfilename);
			...
			String siteFaxNo = request.getParameter("siteFaxNo");
			for (int i = 0; i < recipients.size(); i++) {					
			    ...
			    if(null != existfilename && existfilename.length() > 0){
			    	if(existfilename.contains("Letterhead")){
			    		tempName = "Letterhead-" + faxClinicId + formId + "." + System.currentTimeMillis();
			    	}
			    }
				...
				pw.println(faxNo);
				pw.println(request.getParameter("efmdemographic_no"));
				pw.println(request.getParameter("efmprovider_no"));
				pw.println("Eform");
				pw.println(formId);
				if(siteFaxNo == null){
					siteFaxNo = "";
				}
				siteFaxNo = siteFaxNo.replaceAll("-", "");
				siteFaxNo = siteFaxNo.replaceAll(" ", "");
				pw.println(siteFaxNo);
				pw.close();
				fos.close();
	 ...
	 * @throws DocumentException 
	 */
	public void newFaxForms(String[] numbers, String formId, String providerId) throws DocumentException {
		...
		try {
			...
			for (int i = 0; i < recipients.size(); i++) {					
				...
				PrintWriter pw = new PrintWriter(fos);
				pw.println(faxNo);
				pw.println(request.getParameter("demographic_no"));
				pw.println(request.getParameter("providerId"));
				pw.println("Eform");
				pw.println(formId);
				if(siteFaxNo == null){
					siteFaxNo = "";
				...
				saveFaxJob(faxNo, request.getSession().getAttribute("user").toString(), request.getParameter("providerId"),
						"", defaultFaxModem[0], Integer.valueOf(request.getParameter("demographic_no")),
						tempPdf, defaultFaxModem[1], faxConfigs);
			}
			// Removing the consulation pdf.
			tempFile.delete();
	...
	public static boolean saveFaxJob(String destFaxNo, String curUserNo, String providerNo,
	    ...
	    for (FaxConfig faxConfig : faxConfigs) {
	    	if (faxConfig.getFaxNumber() == null || faxConfig.getFaxNumber().equals(faxNumber)) {
