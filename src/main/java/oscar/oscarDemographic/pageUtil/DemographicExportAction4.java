...
public class DemographicExportAction4 extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		auditLog.add("PROVIDER providerNo=" + loggedInInfo.getLoggedInProviderNo() 
...
//				lastName.setPartQualifier(cdsDt.PersonNamePartQualifierCode.BR);
			...
			if (StringUtils.filled(rosterStatus)) {
				...
				enrolmentHistory.setEnrollmentStatus(cdsDt.EnrollmentStatus.Enum.forString(rosterStatus));
				if (rosterStatus.equals("1")) {
					if (UtilDateUtilities.StringToDate(rosterDate)!=null) {
						enrolmentHistory.setEnrollmentDate(Util.calDate(rosterDate));
					}
				} else {
					if (UtilDateUtilities.StringToDate(rosterTermDate)!=null)
						enrolmentHistory.setEnrollmentTerminationDate(Util.calDate(rosterTermDate));
			...
			for (CaseManagementNote cmn : lcmn) {
				...
				if (exPersonalHistory) {
					...
					if (StringUtils.filled(socHist)) {
						...
						ResidualInformation residualInformation = personalHistory.addNewResidualInfo();
				...
				if (exFamilyHistory) {
					...
					if (StringUtils.filled(famHist)) {
						...
						ResidualInformation residualInformation = fHist.addNewResidualInfo();
						List<ResidualInformation.DataElement> dataElementArray = new ArrayList<ResidualInformation.DataElement>();
						ResidualInformation.DataElement data = residualInformation.addNewDataElement();
						data.setName("Summary");
						data.setDataType("String");
				...
				if (exPastHealth) {
					...
					if (StringUtils.filled(medHist)) {
						...
						ResidualInformation residualInformation = pHealth.addNewResidualInfo();
						List<ResidualInformation.DataElement> dataElementArray = new ArrayList<ResidualInformation.DataElement>();
						ResidualInformation.DataElement data = residualInformation.addNewDataElement();
						data.setName("Summary");
						data.setDataType("String");
				...
				if (exClinicalNotes) {
					...
					if (StringUtils.filled(encounter)) {
						...
						if (StringUtils.filled(cmn.getEncounter_type())) {
							cNote.setNoteType(cmn.getEncounter_type());
						}
						for (CaseManagementIssue isu : sisu) {
							String codeSystem = isu.getIssue().getType();
							if (!codeSystem.equals("system")) {
						...
						if (cmn.getObservation_date()!=null) {
							...
							observationDate = formatDateTime(observationDate, cmn.getObservation_date().toString());
							cNote.addNewEventDateTime().setFullDateTime(observationDate);
						}
						...
						for (CaseManagementNote cm_note : cmn_same) {
							...
							if (StringUtils.filled(cm_note.getProviderNo()) && !Util.isVerified(cm_note) && !participatingProviders.contains(cm_note.getProviderNo())) {
								...
								if (cmn.getUpdate_date()!=null) {
									updateDate = formatDateTime(updateDate, cmn.getUpdate_date().toString());
									noteCreatedDateTime.setFullDateTime(updateDate);
								}
							}
							...
							if (StringUtils.filled(cm_note.getSigning_provider_no()) && cm_note.isSigned()) {
								...
								if (cm_note.getUpdate_date()!=null) {
									reviewDate = formatDateTime(reviewDate, cmn.getUpdate_date().toString());
									noteReviewedDateTime.setFullDateTime(reviewDate);
								}
								else noteReviewer.addNewDateTimeNoteReviewed().setFullDateTime(reviewDate);
							}
						}
					}
				}
			...
			if (exAllergiesAndAdverseReactions) {
				...
				for (int j=0; j<allergies.length; j++) {
					...
					ResidualInformation residualInformation = alr.addNewResidualInfo();
					List<ResidualInformation.DataElement> dataElementArray = new ArrayList<ResidualInformation.DataElement>();
					ResidualInformation.DataElement data = residualInformation.addNewDataElement();
					data.setName("Summary");
					data.setDataType("String");
			...
			for (Map<String, Object> prevMap : prevList) {
				...
				if (Util.isNonImmunizationPrevention(prevType)) {
					...
					ResidualInformation residualInformation = immu.addNewResidualInfo();
					List<ResidualInformation.DataElement> dataElementArray = new ArrayList<ResidualInformation.DataElement>();
					ResidualInformation.DataElement data = residualInformation.addNewDataElement();
					data.setName("Summary");
					data.setDataType("String");
			...
			if (exMedicationsAndTreatments) {
				...
				for (int p = 0; p < arr.length; p++){
					...
					if (arr[p].getNosubs()) {
						medi.setSubstitutionNotAllowed("Y");
						mSummary = Util.addSummary(mSummary, "Substitution not Allowed", "Y");
					}
					...
					if (arr[p].isNonAuthoritative()) {
						medi.setNonAuthoritativeIndicator("Y");
						mSummary = Util.addSummary(mSummary, "Non-Authoritative", "Y");
					}

					if (arr[p].getScript_no() != null) {
					...
					ResidualInformation residualInformation = medi.addNewResidualInfo();
					List<ResidualInformation.DataElement> dataElementArray = new ArrayList<ResidualInformation.DataElement>();
					ResidualInformation.DataElement data = residualInformation.addNewDataElement();
					data.setName("Summary");
					data.setDataType("String");
			...
			if (exAppointments) {
				...
				for (int j=0; j<results.size(); j++) {
					...
					if (ap.getStartTime() != null) {
						apptStartTime.setTime(ap.getStartTime());
					}
					aptm.setAppointmentTime(apptStartTime);
			...
			if (exReportsReceived) {
				...
				for (int j=0; j<edoc_list.size(); j++) {
					...
					if (!f.exists()) {
						...
						dateTimeCal = formatDateTime(dateTimeCal, dateTimeStamp);
						if (UtilDateUtilities.StringToDate(dateTimeStamp, "yyyy-MM-dd HH:mm:ss")!=null) {
							rpr.addNewReceivedDateTime().setFullDateTime(dateTimeCal);
						} else {
						...
						for (int i=0; i<hrm.getReportsReceivedTotal(); i++) {
							...
							if (reportDates!=null) {
								if (reportDates.get("eventdatetime")!=null) {
									...
									reportDate = formatDateTime(reportDate, reportDates.get("eventdatetime").toString());
									rpr.addNewEventDateTime().setFullDateTime(reportDate);
								}
								if (reportDates.get("receiveddatetime")!=null) {
									Calendar receivedDate = Calendar.getInstance();
									receivedDate = formatDateTime(receivedDate, reportDates.get("receiveddatetime").toString());
									rpr.addNewReceivedDateTime().setFullDateTime(receivedDate);
								}
							...
							if (reviewerId!=null && reviewDate!=null) {
								Reports.ReportReviewed reviewed = rpr.addNewReportReviewed();
								reviewed.addNewName();
							...
							for (int j=0; j<hrm.getReportOBRContentTotal(i); j++) {
								...
								Reports.OBRContent obrContent = rpr.addNewOBRContent();
	...
	//create ReadMe.txt & ExportEvent.log
		...
		File auditLogFile = new File(files.get(0).getParentFile(), "AuditLog.log");
		BufferedWriter auditWriter = new BufferedWriter(new FileWriter(auditLogFile));
	...
	*/
	...
	private void exportLabResult(HashMap<String,String> labMea, LaboratoryResults labResults, String demoNo) {
		...
		requestDate = formatDateTime(requestDate, reqDate);
		if (StringUtils.filled(reqDate)) labResults.addNewLabRequisitionDateTime().setFullDateTime(requestDate);
			...
			if (labRoutingInfo != null && labRoutingInfo.get("timestamp") != null && labRoutingInfo.get("provider_no") != null) {
				...
				if (UtilDateUtilities.StringToDate(timestamp, "yyyy-MM-dd HH:mm:ss") != null) {
					...
					if (!"0".equals(lab_provider_no)) {
						...
						timestampCal = formatDateTime(timestampCal, timestamp);
						reviewer.addNewDateTimeResultReviewed().setFullDateTime(timestampCal);
