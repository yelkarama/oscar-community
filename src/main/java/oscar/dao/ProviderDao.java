...
public class ProviderDao extends OscarSuperDao {
	...
	private String [][] dbQueries = new String[][] {
			...
			{"intake_medications","select * from drugs as D where demographic_no = ? and archived = 0 and drugId = (select max(drugId) from drugs where demographic_no = D.demographic_no and archived = 0 and (regional_identifier = D.regional_identifier or customName = D.customName))  and (long_term=1 or (end_date is not null and end_date > now())) ORDER BY rx_date DESC, drugId DESC"},
			{"intake_medications_current", "select * from drugs as D where demographic_no = ? and archived = 0 ORDER BY rx_date DESC, drugId DESC"},
