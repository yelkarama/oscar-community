...
%>
	...
	<tr>
		...
		<td><font size="1" face="Arial, Helvetica, sans-serif"> <a
			href="#"
			onClick="openBrWindow('../../../share/CalendarPopup.jsp?urlfrom=<%=request.getContextPath()%>/billing/CA/BC/billingSim.jsp&year=<%=curYear%>&month=<%=curMonth%>&param=<%=URLEncoder.encode("&formdatebox=document.forms[0].xml_vdate.value")%>','','top=0,left=0,width=430,height=310'); return false;">
		From:</a></font> <input type="text" name="xml_vdate" maxlength="10"
			value="<%=xml_vdate%>" readonly></td>
		<td><font size="1" face="Arial, Helvetica, sans-serif"> <a
			href="#"
			onClick="openBrWindow('../../../share/CalendarPopup.jsp?urlfrom=<%=request.getContextPath()%>/billing/CA/BC/billingSim.jsp&year=<%=curYear%>&month=<%=curMonth%>&param=<%=URLEncoder.encode("&formdatebox=document.forms[0].xml_appointment_date.value")%>','','top=0,left=0,width=430,height=310'); return false;">
