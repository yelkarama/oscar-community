...
function _AddOtherFax(name, number) {
	...
	var remove = "<a href='javascript:void(0);' onclick='removeRecipient(this)'>remove</a>";
        ...
        //splitting the name variable into first name and last name
        var arr = name.split(',');

	jQuery("#textarea2").text(number);
        jQuery("#textfield14").val(arr[0]);
        jQuery("#textfield15").val(arr[1]);
//        $('#specialistsList').val($('#specialistsList').val()  +name+  "\n"   +   number + "\n" );        
