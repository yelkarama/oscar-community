...
/*  editControl - a WYSIWYG edit control using iFrames and designMode
    Copyright (C) 2009-2020 Peter Hutten-Czapski
     Version 1.6 now about 600 lines of code
        ...
        NEW in 2.2 added support for consult_sig_xxx.png pattern stamps
...
jQuery(document).ready(function(){
	...
	if (typeof signatureControl != "undefined") {
		signatureControl.initialize({
			sigHTML:baseUrl()+"/signature_pad/tabletSignature.jsp?inWindow=true&saveToDB=true&demographicNo=",
...
//				seteditControlContents(cfg_editorname, decodeURIComponent(contents));
	...
	function pickStamp() {
		...
		// if current_user_id set in RTL then use the consult_sig_xxx.png pattern
		...
		if (cache.contains("doctor")) {
			for (i=0; i<ImgArray.length;i++){
		        ...
		        if (cache.get('doctor').indexOf(UserName)>=0){
		            mystamp = '<img src="../eform/displayImage.do?imagefile='+FileName+'" width="250" height="80" />';
			    }
			}
		}
		// allow for signing by the current user 
		if (cache.contains("current_user")) {
			for (i=0; i<ImgArray.length;i++){
		        var ListItemArr =  ImgArray[i].split("|");
		        var UserName = ListItemArr[0];
...
function getMeasures(measure, max) {
    ...
    var newURL = baseUrl()+'/oscarEncounter/oscarMeasurements/SetupDisplayHistory.do?type=' + measure;
...
//-- Autocomplete  script --
        ...
        function consultantSearch(term) {
            ...
            //request.open('GET', baseUrl()+'/oscarEncounter/oscarConsultationRequest/searchprofessionalSpecialists.json?keyword=' + term, true);
            request.open('GET', baseUrl()+'/oscarEncounter/oscarConsultationRequest/searchProfessionalSpecialist.json?keyword='+term, true);
