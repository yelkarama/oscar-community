...
function SigWebRefresh() {
    ...
    if (NumPoints > 0 && !isSaved()) {
        setSigning(true);
    }

    document.getElementById("save").style.display = isSigning() ? "inline" : "none";
    document.getElementById("clear").style.display = isSigning() || isSaved() ? "inline" : "none";
    document.getElementById("signMessage").style.display = !isSigning() && !isSaved() ? "inline" : "none";
