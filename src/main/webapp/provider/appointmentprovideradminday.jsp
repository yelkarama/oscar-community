...
if (enhancedOrClassic != null && "E".equals(enhancedOrClassic.getValue())) {
    showAgreement = true;
}
...
function popupInboxManager(varpage){
    ...
    windowprops = "height=900,width=1215,location=no,"
...
<tr>
<td align="center" >
<a href="../web/" title="OSCAR EMR"><img src="../images/oscar_small.png" border="0"></a><!-- <img src="<%=request.getContextPath()%>/images/oscar_small.png" border="0"> -->
</td>
...
<security:oscarSec roleName="<%=roleName$%>" objectName="_appointment.doctorLink" rights="r">
   <li>
       ...
       <oscar:newUnclaimedLab>
				   <% if (enhancedOrClassic != null && "E".equals(enhancedOrClassic.getValue())) { %>
           <a class="tabalert" href="javascript:popupPage(900, 1215, '/kaiemr/app/components/inbox/?providerNo=0')" TITLE='<bean:message key="provider.appointmentProviderAdminDay.viewLabReports"/>'>*</a>
				   <% } else { %>
...
<td class="title noprint" ALIGN="center" width="33%" style="padding:7px"><!-- BGCOLOR="ivory" -->
...
%>
        ...
        <table id="scheduleTable" border="0" cellpadding="0" bgcolor="#486ebd" cellspacing="0" width="100%">
		<thead>
        <tr>
<%
      ...
      iterMax=numProvider;
				...
				<td valign="top" style="width: <%=isWeekView?100/7:100/numProvider%>%">
					<table border="0" cellpadding="0" bgcolor="#fff" cellspacing="0" width="100%">
						<tr>
							<td class="infirmaryView" NOWRAP ALIGN="center" style="padding:10px">
								<logic:notEqual name="infirmaryView_isOscar" value="false">
   ...
   for(int iterNum=0;iterNum<iterMax;iterNum++) {
    ...
    for(Appointment appointment : appointmentsToCount){
     ...
     if(request.getParameter("viewall")==null || request.getParameter("viewall").equals("0") ) {
		 ...
		 if(sd == null|| "0".equals(String.valueOf(sd.getAvailable())) ) {
			 if(nProvider!=me ) userTemplateAvailable = false;
		 }
	 }
...
<security:oscarSec roleName="<%=roleName$%>" objectName="_masterLink" rights="r">
    ...
    &#124; <a class="masterBtn" href="javascript: function myFunction() {return false; }" onClick="popupWithApptNo(700,1024,'../demographic/demographiccontrol.jsp?demographic_no=<%=demographic_no%>&apptProvider=<%=curProvider_no[nProvider]%>&appointment=<%=appointment.getId()%>&displaymode=edit&dboperation=search_detail<%= bMultisites ? "&site=" + sitename : "" %>','master',<%=appointment.getId()%>)"
