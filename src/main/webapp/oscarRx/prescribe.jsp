...
if(listRxDrugs!=null){
  for(RxPrescriptionData.Prescription rx : listRxDrugs){
      ...
      //   boolean isCustomNote   =rx.isCustomNote();
         ...
         String regionalIdentifier = rx.getRegionalIdentifier();
         ...
         if(gcn==0 && StringUtils.isBlank(regionalIdentifier)) {//it's a custom drug
            drugName=customName;
         }else{
            drugName=brandName;
         }
...
<fieldset style="margin-top:2px;width:580px;" id="set_<%=rand%>">
    ...
    <input tabindex="-1" type="text" id="drugName_<%=rand%>"  name="drugName_<%=rand%>"  size="60" <%if(gcn==0 && StringUtils.isBlank(regionalIdentifier)){%> onkeyup="saveCustomName(this);" value="<%=drugName%>"<%} else{%> value='<%=drugName%>'  onchange="changeDrugName('<%=rand%>','<%=drugName%>');" <%}%>/><span id="alleg_<%=rand%>" style="color:red;"></span>&nbsp;&nbsp;<span id="inactive_<%=rand%>" style="color:red;"></span><br>
    ...
    <% if (!Boolean.parseBoolean(OscarProperties.getInstance().getProperty("rx_hide_dispensing_units"))) { %>
    ...
    <% } %>
        <label style="">Repeats:</label><input type="text" id="repeats_<%=rand%>" onblur="addRepeats(this);" <%if(rx.isCustomNote()){%> disabled <%}%>    name="repeats_<%=rand%>"   value="<%=repeats%>" />
            ...
            <div><a>Ingredient:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%=genericName%></a><a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Strength:&nbsp;&nbsp;<%=dosage%></a></div>
                <%} else {%>
            <div>
            <label>Ingredient Name:<input type="text" name="ingredient_name_<%=rand%>" value=""  placeholder="Separate with /"/></label>&nbsp;
            <label>Ingredient Strength: <input type="text" name="ingredient_strength_<%=rand%>" value=""  placeholder="Separate with /"/></label>
        </div>
        <% } %>
...
<fieldset style="margin-top:2px;width:620px;" id="set_<%=rand%>">
    ...
    <label style="float:left;width:80px;" title="<%=ATC%>" >Name:</label>
    <input type="hidden" name="atcCode" value="<%=ATCcode%>" />
    <input tabindex="-1" type="text" id="drugName_<%=rand%>"  name="drugName_<%=rand%>"  size="30" <%if(gcn==0 && StringUtils.isBlank(regionalIdentifier)){%> onkeyup="saveCustomName(this);" value="<%=drugName%>"<%} else{%> value='<%=drugName%>'  onchange="changeDrugName('<%=rand%>','<%=drugName%>');" <%}%> TITLE="<%=drugName%>"/>&nbsp;<span id="inactive_<%=rand%>" style="color:red;"></span>
...
<br />
    ...
    <input type="text" id="instructions_<%=rand%>" name="instructions_<%=rand%>" onkeypress="handleEnter(this,event);" value="<%=instructions%>" size="60" onchange="parseIntr(this);" /><a href="javascript:void(0);" tabindex="-1" onclick="displayMedHistory('<%=rand%>');" style="color:red;font-size:13pt;vertical-align:super;text-decoration:none" TITLE="Instruction Examples"><b>*</b></a>  <a href="javascript:void(0);" tabindex="-1" onclick="displayInstructions('<%=rand%>');"><img src="<c:out value="${ctx}/images/icon_help_sml.gif"/>" border="0" TITLE="Instructions Field Reference"></a> <span id="major_<%=rand%>" style="display:none;background-color:red"></span>&nbsp;<span id="moderate_<%=rand%>" style="display:none;background-color:orange"></span>&nbsp;<span id='minor_<%=rand%>' style="display:none;background-color:yellow;"></span>&nbsp;<span id='unknown_<%=rand%>' style="display:none;background-color:#B1FB17"></span>
       ...
       <div id="siAutoComplete_<%=rand%>" <%if(isSpecInstPresent){%> style="overflow:visible;"<%} else{%> style="overflow:visible;display:none;"<%}%> >
        ...
        <%} else {%>
             <div>
                <label>Ingredient Name:<input type="text" title="" name="ingredient_name_<%=rand%>" placeholder="Separate with /"/></label>&nbsp;
                 <label>Ingredient Strength/Unit: <input type="text" title="" name="ingredient_strength_<%=rand%>" placeholder="Separate with /"/></label>
            </div>
        <%}%>
       <div class="rxStr" title="not what you mean?" >
           <a tabindex="-1" href="javascript:void(0);" onclick="focusTo('method_<%=rand%>')">Method:</a><a   id="method_<%=rand%>" onclick="focusTo(this.id)" onfocus="lookEdittable(this.id)" onblur="lookNonEdittable(this.id);updateProperty(this.id);"><%=methodStr%></a>
