...
<body vlink="#0000FF" class="BodyStyle">
	<div ng-controller="consentConfig">
		<h3>Gateway Interactions Log
		<small ng-repeat="time in expireTimes">{{time.tokenName}} - {{time.expire | date:'medium'}} &nbsp;</small>
		<button class="btn btn-default" ng-click="loadPreviousGatewayLogs();">Load Previous Logs</button>
		</h3>
				...
				<div class="list-group">
						  <a ng-click="openLog(logDetail)" class="list-group-item" ng-class="itemActive(logDetail.id)" data-ng-repeat="logDetail in logDetails | limitTo:loadedSurveillanceConfigsQuantity" >
						  	...
						  	<h4 class="list-group-item-heading">
						  		...
						  		{{logDetail.transactionType}} <small>{{logDetail.externalSystem}} </small>
						  	</h4>
	...
	<script>
		...
		app.controller("consentConfig", function($scope,surveillanceService,providerService,dhdrService) {
			...
			$scope.expireTimes = [];
	    		...
	    		getAllLogs = function(){
	    			dhdrService.getGatewayLogs().then(function(data){
				...
				try{
	    				...
	    				console.log("$scope.currentLogDetail.dataRecievedJson",$scope.currentLogDetail.dataRecievedJson);
				}catch(err){
